...
import { User, Shield, Clock, Calendar, KeyRound, Phone, Hash } from 'lucide-react';
    ...
    <div className="animate-in slide-in-from-bottom-4 fade-in duration-500">
      ...
      <div className="rounded-2xl border border-white/5 bg-gray-900/30 p-6 backdrop-blur-xl">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              <Badge status={profile.estado || 'ACTIVO'} />
            </div>
            <p className="text-sm text-gray-400 mt-0.5">ID: {profile.idUsuario} · {profile.documento}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4">
          ...
          {profile.fechaNacimiento && <DetailField icon={<Calendar size={14} />} label="Fecha Nacimiento" value={profile.fechaNacimiento} />}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        {lastSession && (
          <div className="rounded-2xl border border-white/5 bg-gray-900/30 p-6 backdrop-blur-xl">
            ...
            <span className={`inline-block mt-2 text-xs px-2 py-0.5 rounded-full ${lastSession.activa ? 'bg-emerald-500/10 text-emerald-400' : 'bg-gray-500/10 text-gray-400'}`}>
