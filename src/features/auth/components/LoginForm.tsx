...
export default function LoginForm() {
  const { loginUser, isLoading, error: hookError } = useLogin();
  const location = useLocation();
  const error = hookError || location.state?.error;
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 w-full max-w-sm">
      ...
      <div className="relative mt-6">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="px-2 bg-gray-900/50 text-gray-400">O continuar con</span>
        </div>
      </div>

      <div className="mt-6 flex flex-col gap-3">
        <a
          href={`${import.meta.env.VITE_API_URL || 'http://localhost:3020'}/oauth2/authorization/google`}
          className="w-full flex justify-center items-center gap-3 py-2.5 px-4 border border-white/10 rounded-xl shadow-sm bg-white/5 text-sm font-medium text-white hover:bg-white/10 focus:outline-none transition-colors"
        >
        ...
        <a
          href={`${import.meta.env.VITE_API_URL || 'http://localhost:3020'}/oauth2/authorization/github`}
          className="w-full flex justify-center items-center gap-3 py-2.5 px-4 border border-white/10 rounded-xl shadow-sm bg-white/5 text-sm font-medium text-white hover:bg-white/10 focus:outline-none transition-colors"
        >
